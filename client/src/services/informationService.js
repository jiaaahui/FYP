  ...
  // Adapter that uses your existing client/src/api/Api.js model helpers.
// This expects Api.js to export `building` and `zone` model helpers (your repo does).
//
// - getAllBuildings() -> returns raw rows from /api/building (each row: { id, data, created_at, ... })
// - addBuilding(data) -> sends { data } to POST /api/building; server generates id
// - updateBuilding(id, data) -> sends { data } to PUT /api/building/:id
// - deleteBuilding(id) -> DELETE /api/building/:id
// - getAllZones() -> returns raw rows from /api/zone
//
// Adjust the import path if you move this file.

import { building, zone } from "../api/Api";

// NOTE: Api.* methods throw on non-OK responses (Api.js's handleResponse).
// We just forward errors to calling components.

export async function getAllBuildings() {
  // returns raw rows (each row contains `data` JSON column)
  return building.list();
}

export async function addBuilding(data) {
  // Server auto-generates id for the building. Send { data }
  return building.create({ data });
}

export async function updateBuilding(id, data) {
  // Update the JSON `data` column (server expects the shape matching your Prisma model)
  return building.update(id, { data });
}
...
export async function getAllZones() {
  // returns raw zone rows (each row contains `data` JSON column)
  return zone.list();
}
