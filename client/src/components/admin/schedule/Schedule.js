...
export default function Schedule() {
    ...
    // Reschedule controls / status
    const [rescheduleMode, setRescheduleMode] = useState('now'); // 'now' or 'specific'
    ...
    // --- Scheduler trigger ---
    // Note: The scheduler service that queries Firestore with admin SDK must run server-side (Cloud Function / API).
    // From the browser we trigger it via an HTTP endpoint. The endpoint must be implemented server-side to call the scheduler.
    // Here we POST to /api/scheduler/run with { runAt } where runAt is an ISO timestamp or "now".
    const triggerScheduler = async () => {
        setRescheduleLoading(true);
        setRescheduleMessage('');
        try {
            let runAt;
            if (rescheduleMode === 'now') {
                runAt = new Date().toISOString();
            } else {
                // combine date and hour (time)
                if (!rescheduleDate || !rescheduleHour) {
                    setRescheduleMessage('Please select both date and time.');
                    setRescheduleLoading(false);
                    return;
                }
                // create a Date object in local timezone from date + time
                const iso = new Date(`${rescheduleDate}T${rescheduleHour}:00`);
                runAt = iso.toISOString();
            }

            // Call scheduler backend endpoint - replace URL with your actual scheduler endpoint.
            ...
            if (!resp.ok) {
                ...
                throw new Error(text || `Scheduler endpoint returned ${resp.status}`);
            }

            const json = await resp.json();
            // Expect backend to run scheduler or enqueue job and return result/ack
                ...
                <div className="mb-4 p-3 border rounded-md bg-gray-50">
                    <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <label className="inline-flex items-center gap-2 text-sm">
                                    <input
                                        ...
                                        name="rescheduleMode"
                                        value="now"
                                        checked={rescheduleMode === 'now'}
                                        onChange={() => setRescheduleMode('now')}
                                    />
                                    Now
                                </label>
                                <label className="inline-flex items-center gap-2 text-sm">
                                    <input
                                        ...
                                        name="rescheduleMode"
                                        value="specific"
                                        checked={rescheduleMode === 'specific'}
                                        onChange={() => setRescheduleMode('specific')}
                                    />
                                    Specific time
                                </label>
                            </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            >
                                {rescheduleLoading ? 'Scheduling...' : 'Run Scheduler'}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="mt-2 text-xs text-gray-500">
                        Note: This will trigger the server-side scheduler (server endpoint: POST /api/scheduler/run). The backend should call the scheduler service (admin SDK) to perform assignments.
                    </div>
                </div>
